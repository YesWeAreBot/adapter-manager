...
```
## 快速开始：编写一个业务插件
只需依赖 `adapter-manager`，你就可以写出跨平台的机器人逻辑：
```go
package my_plugin
import (
    "fmt"
...
func (p *MyPlugin) Init(ctx *core.SystemContext) error {
    ...
    ctx.Events.Subscribe("adapter.message", func(payload any) {
        ...
        fmt.Printf("[%s][%s] %s(%d): %s\n", 
            event.Platform, event.Scene, event.Nickname, event.UserID, event.PlainText)
        // 🌟 极简回复：直接调用闭包
        if event.PlainText == "ping" {
            event.ReplyText("pong!")
        }
        // 🌟 跨平台群管操作
        if event.PlainText == "违禁词" {
            event.Delete()           // 撤回消息
            event.BanSender(60)      // 禁言发送者 60 秒
            event.ReplyText("已处理违禁词")
        }
    })
    // 2. 主动调用示例 (定时任务等)
    go func() {
        ...
        if manager != nil {
            // 主动给某个群发消息
            manager.SendGroupMessage("adapter-onebot", 123456789, []adapter_manager.MessageSegment{
                adapter_manager.Text("早安！"),
            })
        }
    }()
...
| `Segments` | `[]MessageSegment` | 结构化消息段数组 |
| `PlainText` | `string` | 纯文本消息（用于指令匹配） |
| `LLMText` | `string` | LLM 友好文本（用于大模型处理） |
| `RawEvent` | `any` | 底层原始事件结构体 |
#### 绑定方法 (闭包)
| 方法 | 参数 | 说明 |
| :--- | :--- | :--- |
| `Reply(msg)` | `[]MessageSegment` | 回复当前消息发送者 (自动判断群聊/私聊) |
| `ReplyText(msg)` | `string` | 快捷纯文本回复 |
| `Delete()` | - | 撤回当前消息 |
...
```
### 3. 消息段 (`MessageSegment`)
`MessageSegment` 是结构化的消息单元，替代传统的 CQ 码字符串。
#### 构造函数
```go
// 文本消息
textSeg := adapter_manager.Text("Hello World")
// @某人
atSeg := adapter_manager.At(123456789)
// 图片消息
imageSeg := adapter_manager.Image("https://example.com/image.png")
// 回复消息
replySeg := adapter_manager.Reply(987654321)
// QQ表情
faceSeg := adapter_manager.Face(123)
```
#### 消息段方法
```go
// 获取字符串参数
url := imageSeg.GetString("url")
// 获取整数参数
userID := atSeg.GetInt64("user_id")
```
### 4. 自定义渲染器
`adapter-manager` 支持自定义消息渲染逻辑，你可以注册自己的渲染器：
```go
// 自定义 LLM 渲染器
type MyLLMRenderer struct{}
func (r *MyLLMRenderer) Render(segments []adapter_manager.MessageSegment) string {
...
// 在插件初始化时注册
func (p *MyPlugin) Init(ctx *core.SystemContext) error {
    adapter_manager.RegisterLLMRenderer(&MyLLMRenderer{})
    return nil
}
```
## 进阶：处理平台特有逻辑
